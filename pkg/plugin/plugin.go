...
type EnvArgs struct {
	...
	K8S_POD_UID cnitypes.UnmarshallableString `json:"K8SPodID,omitempty"`
}
...
func attachIfaceToBridge(ovsDriver *ovsdb.OvsBridgeDriver, hostIfaceName string, contIfaceName string, podID string, ofportRequest uint, vlanTag uint, trunks []uint, portType string, intfType string, contNetnsPath string, ovnPortName string) error {
	err := ovsDriver.CreatePort(hostIfaceName, contNetnsPath, contIfaceName, podID, ovnPortName, ofportRequest, vlanTag, trunks, portType, intfType)
	if err != nil {
		return err
	}
...
func CmdAdd(args *skel.CmdArgs) error {
	...
	if envArgs != nil {
		...
		podID = string(envArgs.K8S_POD_UID)
	}
	...
	if err = attachIfaceToBridge(ovsBridgeDriver, hostIface.Name, contIface.Name, podID, netconf.OfportRequest, vlanTagNum, trunks, portType, netconf.InterfaceType, args.Netns, ovnPort); err != nil {
		return err
	}
	defer func() {
		if err != nil {
