...
func (ovsd *OvsBridgeDriver) CreatePort(intfName, contNetnsPath, contIfaceName, podID, ovnPortName string, ofportRequest uint, vlanTag uint, trunks []uint, portType string, intfType string) error {
	intfUUID, intfOp, err := createInterfaceOperation(intfName, ofportRequest, ovnPortName, intfType)
	if err != nil {
		return err
	}

	portUUID, portOp, err := createPortOperation(intfName, contNetnsPath, contIfaceName, podID, vlanTag, trunks, portType, intfUUID)
	if err != nil {
		return err
	}
...
func createPortOperation(intfName, contNetnsPath, contIfaceName, podID string, vlanTag uint, trunks []uint, portType string, intfUUID ovsdb.UUID) (ovsdb.UUID, *ovsdb.Operation, error) {
